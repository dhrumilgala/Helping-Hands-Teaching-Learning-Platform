  ...
  fetch('/teacher/videos').then((result) => result.json()).then((res) => {
    ...
    if(res.success){
      ...
      for(let i = 0; i < vids.length; i++){
        ...
        a.setAttribute('id',vids[i]._id)
  
        div.appendChild(a);
        let delBtn = document.createElement("button");
        delBtn.id = "delVideo";
        delBtn.className = "delbtn";
        delBtn.innerHTML="DELETE MATERIAL";
...
function submit_video()
{
  fetch('/teacher/addVideo',{
    ...
    if(res.success)
    {
      ...
      videosArticle.innerHTML += "<div><h3>No videos to show</h3></div>";
      swal("Failed!", "The material couldn't be added, kindly contact management.", "error");
    }
  });
}
...
function delete_video(e)
{
  fetch('/teacher/deleteVideo',{
    ...
    body : JSON.stringify(
    {
      link :e.target.previousElementSibling.id
    }),
   headers: {"Content-Type" : "application/json;charset=utf-8"}
  }).then((result) => result.json()).then((res) => {
    console.log(res);
    if(res.success) {
      ...
      swal("Failed!", "Material could not be deleted, contact management", "error");
    }
  });
}
