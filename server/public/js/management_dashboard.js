  ...
  $('body').on("click", "nav ul li a", function(){
    ...
    fetch('/management/donations').then((result) => result.json()).then((res) => {
      ...
      res['donations'].forEach(function(u){
        ...
        p.innerHTML = "Name:"+u["name"]+"</br>"+"Email:"+u["email"]+"</br>"+"Amount Donated:"+u["amountDonated"];
...
function fetch_donation() {
	fetch('/management/donations').then((result) => result.json()).then((res) => {
	  ...
	  for(let key in res){
		console.log(key);
		let p = document.createElement('div');
		p.innerHTML = "Name:"+res[key]["name"]+"</br>"+"Email Id:"+res[key]["email"]+"</br>"+"Amount:"+res[key]["amountDonated"]+"</br>";
		p.innerHTML += "Donations:- </br><ul>";
		res[key]["donation"].forEach((donation) => {
			p.innerHTML += "<li>"+volunteer+"</li>";
		});
		p.innerHTML += "</ul>";
		var button = document.createElement('button');
		var t = document.createTextNode("Remove donation");
		button.appendChild(t);
		button.setAttribute("style", "background-color: red;border:2px solid black;color: white;padding: 15px 32px;text-align: center;text-decoration: none;display: inline-block;font-size: 16px;");
		button.addEventListener("click",function(){
		  fetch('/management/deleteDonation',{
			method : "POST",
			body: JSON.stringify(
			{
			  name : res[key]["name"],
		  ...
		  swal("Deleted!", "The data has been deleted from the database.", "success");
		})
		p.append(button);
		detailsDiv.appendChild(p);
	  }
...
function add_donation() {		// show add donation form
	studentdiv=document.getElementById("div_adddonation");
	studentdiv.style.display="block";
	// alert("potray student add_donation");
}
...
function add_donation_post(name_i, email_i, amount_i){
	fetch('/management/addDonation',{
	  method : "POST",
	  body: JSON.stringify(
	  {
		name : name_i,
		email: email_i,
		amountDonated:amount_i,
	  }),
	  headers: {"Content-Type" : "application/json;charset=utf-8"}
	}).then((result) => result.json()).then((res) => {
	  console.log(res);
	  fetch_donation();
	  show_donations();  // ajax call to reload page not working
	  });
	swal("Added!", "The data has been added to the database", "success");
  }
function submit_donation_add() {
	name=document.getElementById("d_fn").value;
	email=document.getElementById("d_work").value;
	amount = document.getElementById('d_days').value;

	add_donation_post(name, email,amount);
	return false;
